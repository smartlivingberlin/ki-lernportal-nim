...
**Baseline `main`:** `f8bc8a5f1ca60446890e17cd38eb04f6f4f1690e`
...
S51B_C_SCHEMA_AND_MIGRATIONS=AUTHORIZED_DISPOSABLE_LOCAL_ONLY
...
S51D_STAGING_ENVIRONMENT_CREATED=YES
S51D_STAGING_PUBLIC_DOMAIN=ki-lernportal-nim-staging.up.railway.app
S51D_RAILWAY_STAGING_AUTHORIZED=YES_ISOLATED_STAGING_CREATED
...
Wait for CI **ON**. Staging-Environment ist angelegt und HTTP-grün unter
...
`auth_runtime` bleibt Default `false`, keine Login-UI,
CI-Vertrag `pnpm test:s52-c-auth-web-scope`.
...
S52-D2 Login-UI `/anmelden` ist implementiert hinter Flag:
`LOGIN_UI=AUTHORIZED_BEHIND_FLAG`, `LOGIN_UI_IMPLEMENTED=YES`,
`AUTH_RUNTIME_FLAG_FLIP=STAGING_ONLY`, Contracts-Default bleibt `false`.
...
Dashboard-Reverify 2026-08-11 und Staging-Anlage sind in
[S51D_B_STAGING_EXECUTION.md](S51D_B_STAGING_EXECUTION.md) belegt.
Weitere Production-Änderungen brauchen erneut eine Einzelentscheidung.
...
S51B_B_REAL_CONNECTION_AUTHORIZED=YES_DISPOSABLE_LOCAL_ONLY
