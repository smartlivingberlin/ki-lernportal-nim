...
S51B_B_DEPENDENCIES_INSTALLED=drizzle-orm@0.45.2,mysql2@3.23.3
...
## 8. Connection-Proof (Freigabe B, 2026-08-13)

Nachweis nur gegen eine wegwerfbare lokale MySQL-Instanz:

```bash
pnpm test:s51b-b-connection-proof
```

Das Skript startet Docker-MySQL (`ki_nim_s51bb_*`), ruft
`createMySqlRuntime().initialize()` auf, führt `SELECT 1` aus und entfernt den
Container. Kein Schema, keine Migration, kein Railway, keine Production.
Statische URL-Guards laufen in `pnpm test:s51b-b-db-runtime` mit.
