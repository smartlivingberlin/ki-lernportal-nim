...
## 12. Local migrate helper (Freigabe C, 2026-08-13)

Committed SQL only, disposable localhost only:

```bash
DATABASE_URL='mysql://…@127.0.0.1:PORT/ki_nim_s51bc_…' pnpm migrate:s51b-c-local
```

Guards refuse Railway/production hosts and non-`ki_nim_s51bc_` databases.
Does not run `drizzle-kit push`. C2 remains the full disposable Docker proof.
